...
class ScanVFSourceTest {

    private lateinit var server: MockWebServer
    private lateinit var source: ScanVFSource
    ...
    fun setUp() {
        ...
        server.dispatcher = object : Dispatcher() {
            override fun dispatch(request: RecordedRequest): MockResponse {
                ...
                return when {
                    path.startsWith("/manga-list") -> MockResponse().setBody(listHtml)
        ...
        server.start()
        source = ScanVFSource(redirectingClient(server))
    }
    ...
    fun `malformed HTML returns empty list, not an exception`() = runTest {
        ...
        server.start()
        val emptySource = ScanVFSource(redirectingClient(server))
